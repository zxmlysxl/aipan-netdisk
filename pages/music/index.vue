...
const handleDownloadVisible = (song) => {
  ElMessageBox.confirm(
    ...
    "警告（仅供个人学习使用，禁止商业用途）",
    {
      confirmButtonText: "我确定，并且自己承担后果",
      cancelButtonText: "取消",
      type: "warning",
    }
  )
...
<template>
  <div class="max-w-md min-h-screen mx-auto p-[20px]">
    <div class="flex flex-row items-center justify-center gap-3">
      <a href="/">
        <img
          class="w-[40px] h-[40px] sm:w-[40px] sm:h-[40px]"
    ...
    <div class="flex flex-row items-center justify-center gap-2 mt-5">
      ...
      />
      <button
        class="bg-gray-900 text-white px-4 py-2 rounded-md"
        @click="handleSearch()"
      >
        搜索
      </button>
    ...
    <div class="space-y-2 mt-5" v-if="kwData && kwData.length > 0">
      <div
        class="flex flex-row gap-2 items-center justify-between p-2 border-b border-gray-300"
      >
        <h1 class="text-xl font-bold">搜索结果</h1>
        <div class="space-x-2">
          <button
            class="text-gray-950 text-sm hover:underline"
      ...
      <ul class="space-y-2">
        <li
          class="flex flex-row justify-between items-center text-sm p-2 bg-gray-100"
          v-for="(item, index) in kwData"
          :key="index"
        >
          <div>
          ...
          <div class="min-w-[60px] text-right">
            <button
              class="bg-gray-900 text-white px-2 py-1 rounded-md text-xs hover:text-md"
      ...
      <div class="bg-white p-5 rounded-md space-y-3">
        ...
        <div class="space-y-2">
          ...
          <ul class="flex flex-row items-center flex-wrap gap-2">
            <li
              v-for="(q, i) in currentDownloadSong.quality"
              :key="i"
              class="text-xs p-2 rounded-sm cursor-pointer border border-gray-300"
              :class="quality === i ? 'bg-gray-900 text-white' : ''"
              @click="quality = i"
            >
        ...
        <div class="space-x-2 text-center">
          ...
          <button
            class="bg-gray-900 text-white px-2 py-1 rounded-md text-xs hover:text-md"
            @click="handleCopySongUrl(currentDownloadSong)"
          >
            复制歌曲链接
          </button>
          <button
